...
// Vector cross product for doubles (RC)

template<class T>
T operator^ (const Vector<T,2>& A, const Vector<T,2>& B)
{
...
{ std::cerr << "Warning [shared/in/vector.hpp]: vector division by zero\n"; }
...
template<class T, int N>
std::istream& operator>> (std::istream& s, Vector<T,N>& V)
  {
    T x[N];
    char c=0;
    int i;
...
template<class T, int N>
inline Vector<T,N> real(const Vector<std::complex<T>,N>& V)
  { Vector<T,N> R;
    for(int i=0; i<N; i++) R[i] = real(V(i)); 
    return R; }
template<class T, int N>
inline Vector<T,N> imag(const Vector<std::complex<T>,N>& V)
  { Vector<T,N> R;
    for(int i=0; i<N; i++) R[i] = imag(V(i)); 
    return R; }
template<class T, int N>
inline 	Vector<T,N> arg(const Vector<std::complex<T>,N>& V)
  { Vector<T,N> R;
    for(int i=0; i<N; i++) R[i] = arg(V(i)); 
    return R; }
template<class T, int N>
inline 	Vector<std::complex<T>,N> conj(const Vector<std::complex<T>,N>& V)
  { Vector<std::complex<T>,N> R;
