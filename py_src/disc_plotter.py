...
class DiscPlotter:
    ...
    def __init__(self, R_min, R_max, R_interval, N_phi_intervals):
        ...
        self.R_values = np.flip(np.array([R_max - i*R_interval for i in range(N_R_intervals)]))
    ...
    def plotArrows(self, data, N_arrows_R, N_arrows_phi, path):
        ...
        arrow_interval_R = self.N_R_intervals/N_arrows_R
        arrow_interval_phi = self.N_phi_intervals/N_arrows_phi
        
        figure = plt.figure()
        ax = figure.add_subplot(111, polar='True')
        ax.quiver(X[arrow_interval_R::arrow_interval_R, ::arrow_interval_phi],
                  Y[arrow_interval_R::arrow_interval_R, ::arrow_interval_phi],
                  data[arrow_interval_R::arrow_interval_R, ::arrow_interval_phi, 0]
                  *np.cos(X[arrow_interval_R::arrow_interval_R, ::arrow_interval_phi])
                  - data[arrow_interval_R::arrow_interval_R, ::arrow_interval_phi, 1]
                  *np.sin(X[arrow_interval_R::arrow_interval_R, ::arrow_interval_phi]),
                  data[arrow_interval_R::arrow_interval_R, ::arrow_interval_phi, 0]
                  *np.sin(X[arrow_interval_R::arrow_interval_R, ::arrow_interval_phi])
                  + data[arrow_interval_R::arrow_interval_R, ::arrow_interval_phi, 1]
                  *np.cos(X[arrow_interval_R::arrow_interval_R, ::arrow_interval_phi]))
